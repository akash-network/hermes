...
import { PriceUpdateUnordered } from "./price-update-unordered";
import type { PriceUpdate, PriceUpdateOptions } from "../../types";

describe(PriceUpdateUnordered.name, () => {
  it("returns the transaction hash from the broadcast", async () => {
    const { updater, signingClient } = setup();
    signingClient.broadcastTx.mockResolvedValue({ ...deliveredTx, transactionHash: "broadcast-hash" });

    const result = await updater.updatePrice(priceUpdate, options);

    // gasUsed is the buffered fee estimate, not the delivered tx's gasUsed.
  ...
  it("signs with the sender address and broadcasts the signed bytes", async () => {
    ...
    expect(signingClient.broadcastTx).toHaveBeenCalledWith(expectedTx);
  });
  ...
  it("propagates broadcast errors", async () => {
    const { updater, signingClient } = setup();
    signingClient.broadcastTx.mockRejectedValue(new Error("mempool full"));
  ...
  // gasUsed is deliberately different from the buffered fee estimate (150_000) so the
  // assertions show which of the two updatePrice reports.
  const deliveredTx: DeliverTxResponse = {
    height: 42,
    txIndex: 0,
    code: 0,
  ...
  function setup() {
    ...
    signingClient.broadcastTx.mockResolvedValue(deliveredTx);
    ...
    const updater = new PriceUpdateUnordered(signingClient, signer);
